...
def check_subseq_conditions(subseq, m_seq, positions, acids, S):
    match_count = sum(
...
def compare_sequences(m_seqs, n_file, positions1, acids1, k1, S1, positions2, acids2, k2, S2, percent_range,
                      output_file):
    results = []
    ...
    for m_seq in m_seqs:
        for header, n_seq in n_sequences:
            ...
            lengths = [len(m_seq) + i for i in range(-max(k1, k2), max(k1, k2) + 1)]
            ...
            for length in lengths:
                ...
                for start, subseq in enumerate(subsequences, subseq_start):
                    ...
                    # 检查两个条件 (Check two conditions)
                    condition1_met = check_subseq_conditions(subseq, m_seq, positions1, acids1, S1)
                    condition2_met = check_subseq_conditions(subseq, m_seq, positions2, acids2, S2)
    ...
    parameters_data = {
        'Parameter': ['gap_penalty', 'match_score', 'mismatch_penalty', 'k1', 'S1', 'k2', 'S2', 'percent_range',
                      'positions1', 'acids1', 'positions2', 'acids2', 'm_seqs', 'n_file', 'output_file'],
        'Value': [
            gap_penalty, match_score, mismatch_penalty, k1, S1, k2, S2, str(percent_range),
            positions1, acids1, positions2, acids2, ' '.join(m_seqs), n_file, output_file
        ]
    }
...
k1 = 0
S1 = 5
positions1 = [2,3,4,5,6,7,8,9,12]
acids1 = ['A','U','C','A','A','C','G','C','C']
k2 = 0
S2 = 12
positions2 = [1,10,11,13,14,15,16,17,18,19,20,21]
acids2 = ['C','U','G','A','C','U','C','A','A','U','G','A']
percent_range = [0, 100]
m_seqs = ['CAUCAACGCUGCACUCAAUGA']
n_file = r"C:\Users\27404\Arabidopsis_mRNA_library.fa"
output_file = r"C:\Users\27404\Arabidopsis_miR397a_Target.xls"

# 执行函数 (Executive function)
result_file_path = compare_sequences(m_seqs, n_file, positions1, acids1, k1, S1, positions2, acids2, k2, S2,
                                     percent_range, output_file)